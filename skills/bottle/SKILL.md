...
- 小概率会开出彩蛋（空瓶/水母/靴子/Clawd），彩蛋不消耗海里的真瓶子，
  展示后可以顺嘴说一句"再捞一次试试"。
- 捞到真瓶子时把脚本输出原样展示，并提醒可以用 reply 回音。
- 如果海是空的（捞到海草），顺势鼓励用户扔一只。
- 捞完可以顺带提一句：想看动画版可在终端直跑
  `! python3 $SKILL_DIR/scripts/anim.py fish`（仅限有终端的环境，如 Claude Code 的 `!` 前缀）。
...
脚本输出的 ASCII 瓶子卡片依赖等宽对齐，**必须原样放进 ``` 代码块**展示，
一个字符都不要改（对齐是按 CJK 双宽精确计算的）。卡片外可以自由发挥补一两句
话（引导 reply、共情瓶中信内容等）。
